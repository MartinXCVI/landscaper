<template>
  <footer class="footer-element">
    <section class="footer-info">
      <section class="footer-description">
        <h3 class="capital subtitle"><i class="fa-brands fa-pagelines home-intro-icon"></i> Company Name</h3>
        <p class="text">Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente modi neque aliquid quas ut voluptatibus, assumenda dolores? Atque, distinctio cupiditate. Neque sit veniam consequatur numquam in soluta consequuntur blanditiis delectus quis ratione. Eligendi, suscipit voluptatum!</p>
      </section> 
      <section class="footer-nav-wrapper">
        <h3 class="capital subtitle">Navigation</h3>
        <ul class="footer-navbar">
          <li><a href="/">Home</a></li>
          <li><a href="/about">About</a></li>
          <li><a href="/projects">Projects</a></li>
          <li><a href="/services">Services</a></li>
          <li><a href="/contact">Contact</a></li>
        </ul>
      </section>
      <section class="footer-services-wrapper">
        <h3 class="capital subtitle">main services</h3>
        <ul class="footer-services">
          <li><a href="#">Service name</a></li>
          <li><a href="#">Service name</a></li>
          <li><a href="#">Service name</a></li>
          <li><a href="#">Service name</a></li>
          <li><a href="#">Service name</a></li>
          <li><a href="#">Service name</a></li>
        </ul>
      </section>
      <section class="footer-contact">
        <h3 class="capital subtitle">Contact us</h3>
        <div class="footer-contact-div">
          <h4 class="capital"><i class="fa-regular fa-clock"></i> we're open</h4>
          <p>Mon - Fri: 08:30 - 17:30</p>
        </div>
        <div class="footer-contact-div">
          <h4><i class="fa-solid fa-location-dot"></i> Location</h4>
          <p class="capital">123 street, city - state</p>
        </div>
        <div class="footer-contact-div">
          <h4><i class="fa-solid fa-envelope"></i> Email us</h4>
          <p>company@domain.com</p>
        </div>
      </section>
    </section>
    <figure class="footer-bottom-fig">
      <img src="/images/footer-bottom.png" alt="">
    </figure>
    <p class="dev-signature">
      Developed by <a class="github-link" href="https://github.com/MartinXCVI" target="_blank">MartinXCVI <i class="fa-brands fa-github"></i></a>
    </p>
  </footer>
</template>